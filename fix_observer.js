...
const newScript = const fadeObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      e.target.classList.add("in");
    } else if (e.boundingClientRect.top > 50) {
...
for (const file of files) {
    ...
    // index.html uses single quotes in the original script
    ...
    if (content.match(oldScript) || content.match(oldScriptIndex)) {
        ...
        console.log('Observer script not found or already fixed in ' + file);
    }
}