...
.stage{
  ...
  justify-content:center;padding:10px 12px;position:relative;z-index:2;
}
...
.book-wrap.single .face-in{padding-left:54px;padding-right:1.1em}
...
/* vùng chạm lật trang */
.flip-zone{position:absolute;top:0;bottom:0;width:16%;z-index:70;cursor:pointer;background:transparent;border:none;padding:0;touch-action:none}
...
.flip-hint{
  position:absolute;bottom:12px;font-family:'Shantell Sans',cursive;font-size:.82em;color:#7d7260;
  background:rgba(255,253,246,.75);padding:3px 9px;border-radius:6px;border:1px dashed rgba(125,114,96,.3);
  opacity:.85;transition:.2s;pointer-events:none;
}
.flip-zone.next .flip-hint{right:12px}
.flip-zone.prev .flip-hint{left:12px}
.book-wrap.single .flip-zone.prev .flip-hint{left:52px}
.flip-zone:hover .flip-hint{opacity:1;background:#ffdf6b;color:#5d4a12;border-style:solid;transform:scale(1.06)}
...
/* ═══ NÚT ĐIỀU HƯỚNG TRÊN BÌA SỔ (TRANG ĐẦU) ═══ */
.cov-tools{position:absolute;top:10px;right:12px;display:flex;align-items:center;gap:6px;z-index:25}
.cov-tool-btn{
  background:rgba(255,255,255,.8);border:1.5px solid rgba(80,55,20,.28);border-radius:999px;
  padding:3px 9px;font-family:'Shantell Sans',cursive;font-size:.78rem;color:#4a3818;
  cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:4px;
  box-shadow:1px 2px 0 rgba(43,32,10,.15);transition:.15s;
}
.cov-tool-btn:hover{background:#ffdf6b;transform:scale(1.06)}
.cov-nav{display:flex;flex-wrap:wrap;gap:7px;justify-content:center;margin:.7em auto .5em;max-width:94%;position:relative;z-index:10}
.cov-btn{
  font-family:'Shantell Sans',cursive;font-size:.82em;font-weight:700;color:#2e3d59;
  background:#fffdf6;border:2px dashed rgba(46,61,89,.35);border-radius:999px;
  padding:.32em .8em .4em;cursor:pointer;box-shadow:2px 2px 0 rgba(43,32,10,.18);
  transition:.15s;text-decoration:none;display:inline-flex;align-items:center;gap:3px;
}
.cov-btn:hover{
  background:#ffdf6b;border-style:solid;border-color:#5d4a12;color:#5d4a12;
  transform:translateY(-2px) scale(1.04);box-shadow:2px 4px 0 rgba(43,32,10,.25);
}
.cov-btn:active{transform:translate(1px,2px);box-shadow:0 0 0}
.cov-open-btn{display:inline-block;margin:.35em 0 .3em;font-size:1.02em;padding:.5em 1.3em .6em;border-radius:999px;font-weight:700}
...
.cov-burst{width:5.2em;margin:.4em auto .15em;filter:drop-shadow(0 6px 10px rgba(30,16,2,.35))}
.cov-hint{
...
.cov-foot{margin-top:.6em;font-size:.78em;color:#cdbb96;font-style:italic}
...
(function(){
  ...
  function updateUI(){
    ...
    if (nextBtn) nextBtn.disabled = flipped >= L;
    var label;
    if (mode === 'spread'){
    ...
    if (zonePrev) zonePrev.style.display = flipped <= 0 ? 'none' : 'block';
    if (zoneNext) zoneNext.style.display = flipped >= leaves.length ? 'none' : 'block';
  }
  ...
  function layout(){
    ...
    var availW = Math.min(vw - 20, 1180);
    var availH = Math.max(340, vh - 24);
    var pw, ph;
    if (mode === 'spread'){
      pw = Math.min(availW / 2 - 4, 520, availH / 1.30);
      ph = Math.min(pw * 1.34, availH);
      pw = Math.min(pw, ph / 1.22);
    } else {
      pw = Math.min(availW - 14, 490, availH / 1.38);
      ph = Math.min(pw * 1.40, availH);
      pw = Math.min(pw, ph / 1.26);
    }
  ...
  /* ── điều khiển ── */
  var zNext = document.getElementById('zoneNext');
  var zPrev = document.getElementById('zonePrev');
  ...
  function syncSnd(){ if (sndBtn) sndBtn.textContent = sndOn ? '🔊' : '🔇'; }
  if (sndBtn){
